...
# ================================================================
# 2. NUEVA ARQUITECTURA: 3→4→1
# ================================================================
class MediumNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = nn.Linear(3, 4)  # 3 entradas → 4 neuronas ocultas
        self.act = nn.ReLU()
        self.fc2 = nn.Linear(4, 1)  # 4 neuronas → 1 salida
    
    def forward(self, x):
        x = self.act(self.fc1(x))  # Capa oculta con activación
        x = self.fc2(x)            # Capa de salida (sin activación para regresión)
        return x
...
def train_model(dataset, epochs=100, batch_size=512, lr=1e-3):
    ...
    model = MediumNet().to(device)  # Usar la nueva arquitectura
...
def export_weights(model, file_path="pesos_red_3_4_1.csv"):
    rows = []
...
def main():
    ...
    print("Arquitectura: 3→4→1")
    print(f"Parámetros totales: {sum(p.numel() for p in MediumNet().parameters())}")
    ...
    print("Predicciones guardadas en: predicciones_3_4_1.csv")
